...
def match_for_color(new_code, new_guess):
    ...
    return result, sec_guess, sec_code



def hint(code, guess):
    result = look_for_exact(code, guess)
    result = result + match_for_color(code, guess)
    return result
...
def main():
    code = ""
    guess = ""
    print(hint( code, guess) )
    
guess()
look_for_exact(code, guess)
